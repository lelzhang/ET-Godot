...
namespace ET
{
    ...
    public sealed class EventSystem : IDisposable
    {
        ...
        public void Init()
        {
            ...
            //tps.AddRange(ETMono.MonoHelper.Hotfix.GetHotfixTypes());
            //Game.EventSystem.AddRangeType(tps);

#if ILRuntime
            this.types.Clear();

            List<Type> ts = ETMono.MonoHelper.Hotfix.GetHotfixTypes();
            Log.Info(ts.Count.ToString());
            foreach (Type attType in ts)
            {
                foreach (Type type in allAssembliesTypes)
                {
                    ...
                    {
                        BaseAttribute baseAttribute = obj as BaseAttribute;
                        if (baseAttribute != null)
                        {
                            if(baseAttribute.GetType().IsSubclassOf(typeof(BaseAttribute)))
                            Log.Debug(baseAttribute.GetType().Name);
                            this.types.Add(baseAttribute.GetType(), type);
                        }
                    }
            ...
            foreach (Type type in this.GetTypes(typeof(ObjectSystemAttribute)))
            {
                ...
                if (obj is ISystemType iSystemType)
                {
                    ...
                    //Log.Info($"Reg System : {obj.GetType().Name}");
            ...
            foreach (Type type in types[typeof(EventAttribute)])
            {
                ...
                //Log.Info($"Reg Event : {obj.GetType().Name}");
            ...
            //Log.Debug(typeof(EventSystem).Assembly.FullName);//这里程序集为打包加载的
#endif
        ...
        public HashSet<Type> GetTypesHashSet(Type systemAttributeType)
        {
            ...
            foreach (var item in this.types[systemAttributeType])
            {
                types.Add(item);
            }
        ...
        public async ETTask PublishAsync<T>(T a) where T : struct
        {
            ...
            ListComponent<ETTask> list = ListComponent<ETTask>.Create();
            {
        ...
        public async ETTask PublishClassAsync<T>(T a) where T : DisposeObject
        {
            List<object> iEvents;
            if (!this.allEvents.TryGetValue(a.GetType(), out iEvents))
            {
            ...
            {
                object obj = iEvents[i];
                IEventClassAsync aEvent = (IEventClassAsync)obj;
